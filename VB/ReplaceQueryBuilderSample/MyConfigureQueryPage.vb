...
#Region "Overrides of ConfigureQueryPage<TModel>"
        ...
        Protected Overrides Sub RunQueryBuilder()
            ...
            Using form = New FrmQueryBuilder()
                ...
                End If
                Dim query As New TableQuery(tableName)
                query.AddTable(tableName).SelectColumns(dbTable.Columns.Select(Function(c) c.Name).ToArray())
                SetTableOrCustomSqlQuery(query)
                schema = query.GetDataSchema(DBSchema)
                RaiseChanged()
            End Using
        End Sub
