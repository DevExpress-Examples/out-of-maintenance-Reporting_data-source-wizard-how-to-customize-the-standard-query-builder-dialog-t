...
namespace ReplaceQueryBuilderSample {
    class MyConfigureQueryPage<TModel> : ConfigureQueryPage<TModel> where TModel : ISqlDataSourceModel {
        ...
        public MyConfigureQueryPage(IConfigureQueryPageView view, IWizardRunnerContext context, SqlWizardOptions options, 
        ...
        protected override void RunQueryBuilder() {
            // Display a custom query builder form and generate a query based on user input.
            using (var form = new FrmQueryBuilder()) {
                ...
                dataSchema = SqlQueryHelper.GetDataSchema(query, DBSchema, Model.DataConnection, new ConnectionOptions(), 
                    parameterService == null ? null : parameterService.Parameters);
